...
__command_rprompt() {
  local right_text="$(date +%I:%M\ %p)" # Example text: current time
  # Calculate the number of spaces needed for alignment
  local n=$(( COLUMNS - ${#right_text} ))
  # Print the spaces and the text, then a carriage return
  printf "%${n}s%s\r" "" "$right_text"
}
...
function __setprompt
{
	...
	if [[ $LAST_COMMAND != 0 ]]; then
		# PS1="\n\[${RED}\](\[${LIGHTRED}\]ERROR\[${RED}\])-(\[${LIGHTRED}\]Exit Code \[${WHITE}\]${LAST_COMMAND}\[${RED}\])-(\[${LIGHTRED}\]"
	...
	# Date
	PS1+="\n\[${DARKGRAY}\](\[${CYAN}\]\$(date +%a) $(date +%b-'%-m')" # Date
...
# add right side prompt to line about prompt
